...
Requires `dt` 0.3.2. Tool calls stop stacking: a run of thirty greps leaves
one line, not thirty.

### Changed

- **Tool calls share the live activity line again.** Each call overwrites the
  seat's shimmering line instead of leaving a persistent row behind, and the
  line clears when real output streams — reverting the persistent tool rows
  0.3.1 introduced.
